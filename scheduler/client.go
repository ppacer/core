...
func (c *Client) TriggerDagRun(in api.DagRunTriggerInput) error {
	return httpPost[api.DagRunTriggerInput](
		c.httpClient, c.triggerDagRunUrl(), in,
	)
}

// RestartDagRun restarts given DAG run.
func (c *Client) RestartDagRun(in api.DagRunRestartInput) error {
	return httpPost[api.DagRunRestartInput](
		c.httpClient, c.restartDagRunUrl(), in,
	)
}
...
func (c *Client) restartDagRunUrl() string {
	suffix := c.routes[api.EndpointDagRunRestart].UrlSuffix
	return fmt.Sprintf("%s%s", c.schedulerUrl, suffix)
}
